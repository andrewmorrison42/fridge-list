...
### v23.9 — an audit, and three clocks that were not what they claimed

Not a bug report from the family this time: an audit against the invariants in this repo,
asked for because issues were "drifting in". Five fixes, and the three that matter share
one shape — **a value stood in for a fact it was not**.

**A phone told itself it was behind its own write.** `shoppingRemoteModifiedSeen` advanced
in `mergeRemoteShopping` and nowhere else. A successful write moves the folder's mtime; the
fast poll then sees that mtime, records it as *latest*, and returns before merging —
because it recognises it as the copy this device sent. So the two halves of the comparison
could never meet again, and `listFreshness` read `behind` from that poll onwards for ever.
v23.8 had just finished arguing that red must stay rare; this made red the resting state of
any phone somebody was shopping from, and repainted the tab under their thumb as it
appeared. The fix is that a copy you hold — because you read it, or because you sent it —
is one you have seen.

The first attempt at that fix was wrong in an instructive way, and it is recorded because
the reasoning is the reusable part. It also stamped the poll's unchanged-stamp shortcut as
"seen", on the grounds that an mtime the poll recognises is one this device holds. It is
not: `shoppingFastPollStamp` is set *before* the download, so a poll whose fetch then fails
leaves it ahead of anything actually merged — and that is the one case `behind` exists to
report. **Two variables that agree on the happy path are not the same variable.** Only a
holder may claim a copy, which is now two callers and a test that counts them.

**`basedOn` reversed its own rule.** This is the entry worth reading twice, because it
reverses what v21.8 thought it was writing. `weekPlan.basedOn` exists to answer "how
caught-up was the device that generated this list", and it is the only input to
`chooseTripWinner`'s third rule. Five sites wrote it as
`shoppingSeenRemoteAt || shoppingData.lastUpdated`, and `saveShoppingLocal` bumps
`lastUpdated` on every save. So a phone that had never once read the shared folder wrote
the *newest* `basedOn` in the household, and rule 3 — "the stale device losing on the only
ground that distinguishes it" — handed it the win over every phone that had.

That is exactly the defect v23.2 fixed in `effectiveAddedAt`, in a different field, and it
survived the release that fixed it. The rule now: **a device with no horizon makes no
claim**, the same way a device with no horizon can delete nothing. `syncHorizon()` is the
one place that decides, and a falling back to a local save time is what it exists to stop.

**"Keep this one" superseded whatever was stashed.** v23.6 established that a decision
which does not supersede is a pause. True — when another phone is still offering the
losing list. A stash appears three ways, and in two of them there is nothing out there to
outrank: when this phone *lost*, the list on screen already IS the other phone's trip, so
the mint superseded a dead local trip and put a *third* id into a household arguing about
two. A rule stated for one case had been applied to all of them without anyone checking
which cases existed.

Two smaller ones: a 404 from the shopping poll was counted as "cannot reach the folder"
when `fetchRemoteShopping` had always read it as "not created yet" — the same status
meaning two things in two functions one screen apart; and the disagreement banner was
cleared only when there was no conflict at all, so a fork whose loser had no work on it
left a settled argument on screen.

**What the audit says about the tests.** Every one of these sat under a green suite, and
`basedOn` was *heavily* tested — by tests that all handed the value in as a literal, so
nothing ever asked where it came from. That is the "How to review this codebase" point
below, arriving on schedule: coverage measures agreement between a test and the code it was
written beside. Each fix here landed with a test, and each test was run against the
unfixed file first. Two of them did not fail — one because the revert was wrong and one
because the assertion passed vacuously on missing code — which is the entire argument for
doing that check rather than assuming.

**What was deliberately not fixed.** Two findings were left, and both are recorded rather
than quietly dropped:

- The **shared-folder backend** has no merge-on-write, no poll, and no freshness strip.
  It is the pre-v15 behaviour under a modern UI. Left because nobody uses it; written into
  `CLAUDE.md`'s known limitations so it is not mistaken for covered.
- The **v23.7 Wait List rebuild lives inside `renderReviewTab`**, so a background poll that
  merges somebody's Wait List addition calls `generateShoppingList()` — which stamps
  `lastPlanned` and calls `persist('recipes')`, a ~1 MB upload on supermarket mobile data,
  which is precisely what threading `which` through `persist` exists to prevent. The
  exception is still safe by construction against minting a trip or dropping a tick; the
  cost is data and a render that mutates synced state. Fixing it is a shape decision —
  hoist it into the merge/poll path, or give it a rebuild that does not re-upload the
  recipes — and the source test pins the call site, so it is not a drive-by. **Open work,
  not a known limitation:** it should be fixed, just not alongside a sync audit.
