...
npm run test:browser    # 107 browser assertions — needs playwright-core + Chromium
npm run test:all
```
...
trip (which carries ticks across) or there is no progress to lose; otherwise it waits
for a deliberate tap. `sameTripRebuild()` is shared with `generateShoppingList` so the
two can never disagree about what is about to happen. *(v21.8)*
